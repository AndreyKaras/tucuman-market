...
`NotificationOutbox` хранит задание Telegram-уведомления отдельно от заказа:

- ID заказа и тип события
- уникальный ключ дедупликации
- статус и количество попыток
- время следующей попытки и безопасное описание последней ошибки
- временные метки создания, обработки и завершения

Заказ и outbox-задание создаются в одной транзакции. Обработчик повторных
попыток идемпотентен и не хранит секрет Telegram в базе данных.
