...
const fieldsetClass = 'm-0 flex flex-col gap-0.5 border-0 px-0 py-[18px]';
...
export function CatalogControls({ categories, categorySlug, query }: CatalogControlsProps) {
  ...
  const pushQuery = (next: CatalogQuery) => {
    ...
    if (categorySlug) {
      const selectedCategory = categories.find((category) => category.key === cleaned.category);
      ...
      <fieldset className={fieldsetClass}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="text-xs text-ink-muted">
            ...
            <input
              className="mt-1.5 h-11 w-full min-w-0 rounded-[7px] border border-line px-2.5 focus:border-primary-700 focus:shadow-[0_0_0_2px_rgba(20,108,46,0.14)] focus:outline-0"
              defaultValue={query.minPrice}
              inputMode="numeric"
              key={`${id}-min-${query.minPrice}`}
              min="0"
              name="minPrice"
              placeholder="0"
              step="1"
  ...
  return (
    <div className="contents">
      <aside
        className="col-start-1 row-start-1 min-w-0 self-start rounded-xl border border-line p-[18px] motion-safe:animate-[filters-in_220ms_cubic-bezier(0.2,0.8,0.2,1)_both] max-[900px]:hidden"
        aria-labelledby="desktop-filters-title"
      >
        <h2
          className="mt-0 mb-5 flex items-center gap-2 text-lg [&_svg]:size-5"
          id="desktop-filters-title"
        >
          <SlidersIcon />
          {t('filters')}
        </h2>
      ...
      >
        <button
          className={cn(
            secondaryButtonClass,
            'min-[900px]:hidden max-[639px]:w-full [&_span]:inline-flex [&_span]:h-[22px] [&_span]:min-w-[22px] [&_span]:items-center [&_span]:justify-center [&_span]:rounded-full [&_span]:bg-primary-700 [&_span]:text-[11px] [&_span]:text-white',
          )}
          onClick={() => {
        ...
        <label className="flex min-w-0 items-center gap-2.5 max-[639px]:w-full max-[639px]:justify-between">
          ...
          <span className="relative inline-flex max-[639px]:flex-1">
            <select
              ...
              name="sort"
              onChange={(event) =>
                pushQuery({ ...query, sort: event.target.value as CatalogQuery['sort'] })
              }
              value={query.sort}
            >
        ...
        <section className="filter-dialog__panel ml-auto grid h-dvh w-[min(420px,calc(100%-48px))] max-w-[420px] translate-x-full grid-rows-[auto_minmax(0,1fr)] bg-surface shadow-[-12px_0_32px_rgba(23,32,26,0.18)] transition-transform duration-[220ms] ease-[cubic-bezier(0.2,0.8,0.2,1)] max-[639px]:mt-auto max-[639px]:ml-0 max-[639px]:h-[min(86dvh,720px)] max-[639px]:w-full max-[639px]:max-w-none max-[639px]:translate-x-0 max-[639px]:translate-y-full max-[639px]:rounded-t-2xl">
          <header className="flex items-center justify-between border-b border-line px-[22px] py-[18px]">
            <div>
              ...
              <h2 className="mt-0.5 mb-0 text-2xl" id="mobile-filters-title">
                {t('filters')}
              </h2>
            </div>
            <button
