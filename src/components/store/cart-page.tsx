  ...
  if (!cart.state.hasHydrated) {
    return (
      ...
      >
        ...
        <div
          className="mt-8 h-56 animate-pulse rounded-xl bg-surface-muted motion-reduce:animate-none"
          aria-hidden="true"
        />
      </main>
    );
    ...
    <main className={cn(containerClass, 'py-14 max-[639px]:py-8')} id="main-content">
      ...
      {cart.state.items.length === 0 ? (
        <section
          className={cn(
            emptyStateClass,
            'my-16 rounded-xl border border-line px-6 py-16 max-[639px]:my-10 max-[639px]:py-12',
          )}
        >
        ...
        <div className="mt-9 grid grid-cols-[minmax(0,1fr)_360px] items-start gap-8 max-[900px]:grid-cols-1 max-[639px]:mt-7 max-[639px]:gap-6">
          ...
          >
            <div className="flex items-center justify-between border-b border-line px-6 py-5 max-[639px]:px-4">
              <h2 className="m-0 text-lg" id="cart-items-title">
                {t('items')}
              </h2>
          ...
          <aside
            className="sticky top-[116px] rounded-xl border border-line bg-white p-6 max-[900px]:static max-[639px]:p-5"
            aria-labelledby="cart-summary-title"
          >
            <h2 className="mt-0 mb-6 text-xl" id="cart-summary-title">
              {t('summary')}
            </h2>
            <div className="flex items-center justify-between border-y border-line py-5">
              <span>{t('subtotal')}</span>
              <strong className="text-2xl text-primary-700">
