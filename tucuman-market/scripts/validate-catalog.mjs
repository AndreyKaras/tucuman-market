...
invariant(
  productCatalog.priceFormat === 'decimal-string',
  'catalog prices must use decimal strings',
);
...
for (const category of categories) {
  ...
  invariant(
    ...
    `${category.key}: invalid category image width`,
  );
  invariant(
    Number.isInteger(image.height) && image.height > 0,
    `${category.key}: invalid category image height`,
  );
  invariant(
    image.spritePosition === undefined ||
      (Number.isInteger(image.spritePosition) &&
        image.spritePosition >= 0 &&
        image.spritePosition <= 9),
  ...
  for (const locale of ['es', 'en']) {
    ...
    invariant(
      image.translations?.[locale]?.alt,
      `${category.key}: missing ${locale} category image alt`,
    );
  }
}
...
function moneyToMinorUnits(value, label) {
  invariant(
    typeof value === 'string' && moneyPattern.test(value),
    `${label}: invalid decimal money string`,
  );
...
for (const product of products) {
  ...
  invariant(
    Number.isFinite(product.quantityStep) && product.quantityStep > 0,
    `${product.sku}: invalid quantityStep`,
  );
  invariant(
    Number.isFinite(product.stockQuantity) && product.stockQuantity >= 0,
    `${product.sku}: invalid stockQuantity`,
  );
  invariant(
    Number.isFinite(product.lowStockThreshold) && product.lowStockThreshold >= 0,
    `${product.sku}: invalid lowStockThreshold`,
  );

  if (product.netContent !== null) {
    invariant(
      Number.isFinite(product.netContent.value) && product.netContent.value > 0,
      `${product.sku}: invalid net content value`,
    );
    invariant(
      allowedContentUnits.has(product.netContent.unit),
      `${product.sku}: invalid net content unit`,
    );
  }
  ...
  for (const image of product.images) {
    ...
    invariant(
      Number.isInteger(image.width) && image.width > 0,
      `${product.sku}: invalid image width`,
    );
    invariant(
      Number.isInteger(image.height) && image.height > 0,
      `${product.sku}: invalid image height`,
    );
    invariant(
      Number.isInteger(image.sortOrder) && image.sortOrder >= 0,
      `${product.sku}: invalid image sortOrder`,
    );
    invariant(
      !imageOrders.has(image.sortOrder),
      `${product.sku}: duplicate image sortOrder ${image.sortOrder}`,
    );
