...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const category = getCatalog(locale).categories.find(
    (item) => item.key === product.categoryKey,
  );
  const discount = getDiscountPercentage(product);
  const unit = product.saleUnit === "KG" ? t("perKilogram") : t("each");
  const netContent = product.netContent
    ? formatNetContent(product.netContent, locale)
    : unit;

  return (
    <main className={cn(containerClass, "pt-7 pb-20 max-[639px]:pt-5 max-[639px]:pb-14")} id="main-content">
      <p className="m-0 flex items-center gap-2 text-[13px] text-ink-muted [&_a]:inline-flex [&_a]:min-h-11 [&_a]:min-w-11 [&_a]:items-center [&_a:hover]:text-primary-700 [&_a:hover]:underline">
        ...
        {category ? (
          <>
            <span aria-hidden="true">/</span>
            <Link href={{ pathname: "/categories/[slug]", params: { slug: category.slug } }}>{category.name}</Link>
          </>
        ) : null}
        <span aria-hidden="true">/</span>
      ...
      <article className="mt-[26px] grid grid-cols-[minmax(0,1.05fr)_minmax(340px,0.95fr)] gap-[clamp(32px,5vw,72px)] max-[900px]:grid-cols-1 max-[900px]:gap-8 max-[639px]:mt-[18px]">
        ...
        <section className="self-start">
          <div className="flex min-h-[25px] flex-wrap gap-2">
            {discount ? <span className={saleBadgeClass}>{t("discount", { discount })}</span> : null}
          ...
          <h1 className="mt-3.5 mb-2.5 text-[clamp(32px,4vw,48px)] leading-[1.05] tracking-[-0.035em] max-[639px]:text-[32px]">{product.name}</h1>
          <p className="m-0 flex flex-wrap gap-x-[18px] gap-y-2 text-[13px] text-ink-muted">
            {product.brand ? <span>{t("brand")}: <strong>{product.brand}</strong></span> : null}
            <span>{t("sku")}: <strong>{product.sku}</strong></span>
          </p>
          <div className="mt-6 flex flex-wrap items-baseline gap-2.5">
            <strong className="text-[34px] text-primary-700">{formatMoney(product.price, locale)}</strong>
            {product.compareAtPrice ? <s className="text-ink-muted">{formatMoney(product.compareAtPrice, locale)}</s> : null}
            <span className="text-ink-muted">{unit}</span>
          </div>
          <p className={cn("mt-2.5 mb-0 text-sm font-bold text-primary-700", product.isOutOfStock && "text-danger")}>
          ...
          <dl className="my-6 grid grid-cols-2 gap-3.5 border-y border-line py-[18px] [&_div]:min-w-0 [&_dt]:text-xs [&_dt]:text-ink-muted [&_dd]:mt-1.5 [&_dd]:mb-0 [&_dd]:text-sm [&_dd]:font-[650]">
            <div><dt>{t("saleUnit")}</dt><dd>{unit}</dd></div>
            <div><dt>{t("netContent")}</dt><dd>{netContent}</dd></div>
          </dl>
