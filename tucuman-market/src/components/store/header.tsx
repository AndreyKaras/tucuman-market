...
export function Header() {
  ...
  const [recentSearches, setRecentSearches] = useState<string[]>(() => {
    ...
    try {
      return parseRecentSearches(
        window.localStorage.getItem(getRecentSearchesStorageKey(locale)),
      );
    } catch {
      return [];
    }
  ...
  const switchLocale = async (nextLocale: StoreLocale) => {
    ...
    if (
      localizedSlug &&
      (localizedSection === "products" || localizedSection === "productos")
    ) {
      const { getLocalizedProductSlugs, getProductBySlug } = await import(
        "@/features/catalog/data/catalog-repository"
      );
    ...
    if (
      localizedSlug &&
      (localizedSection === "categories" || localizedSection === "categorias")
    ) {
      const { getCatalog, getLocalizedCategorySlugs } = await import(
        "@/features/catalog/data/catalog-repository"
      );
      ...
      if (slug) {
        router.replace(
          { pathname: "/categories/[slug]", params: { slug }, query: Object.fromEntries(searchParams.entries()) },
          { locale: nextLocale },
        );
        return;
      }
  ...
  const isOffersActive =
    pathname === "/products" && searchParams.get("onSale") === "1";
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-30 border-b border-line bg-white/98">
        ...
        >
          <button
            ...
            className={cn(iconButtonClass, "min-[901px]:hidden")}
          ...
          <nav aria-label={t("primary")} className="flex items-center gap-5 max-[1120px]:gap-3 max-[900px]:hidden">
            <Link
          ...
          >
            ...
            >
              ...
              >
                ...
                <span className="hidden text-lg max-[380px]:inline" aria-hidden="true">→</span>
              </button>
            </div>
            {isSearchFocused && recentSearches.length ? (
              <div className="absolute top-full right-0 left-0 mt-1.5 overflow-hidden rounded-[10px] border border-line bg-surface shadow-[0_12px_28px_rgba(23,32,26,0.14)]">
                <div className="flex items-center justify-between px-3.5 pt-2.5 pb-1.5">
                  <strong className="text-xs text-ink-muted uppercase">{catalog("recentSearches")}</strong>
                  <button className="min-h-9 bg-transparent px-1.5 text-xs text-primary-700" onClick={clearRecentSearches} type="button">
      ...
      <div
        ...
        className="group invisible fixed inset-0 z-[80] block pointer-events-none opacity-0 transition-[opacity,visibility] duration-[140ms] ease-out data-[open=true]:visible data-[open=true]:pointer-events-auto data-[open=true]:opacity-100 min-[901px]:hidden"
