...
export function ProductPurchase({ product }: { product: CatalogProduct }) {
  ...
  return (
    <div className="mt-7 grid grid-cols-[auto_minmax(0,1fr)] items-end gap-3.5 max-[639px]:grid-cols-1 max-[639px]:items-stretch">
      <div>
        <span className="mb-[7px] block text-[13px] font-[650]" id="product-quantity-label">{t("quantity")}</span>
        <div className={cn(quantityControlClass, "max-[639px]:w-max")} aria-labelledby="product-quantity-label">
          <button
            aria-label={t("decreaseQuantity")}
            className={quantityButtonClass}
            disabled={quantity <= product.quantityStep}
            onClick={() => setQuantity((value) => Math.max(
              product.quantityStep,
              Number((value - product.quantityStep).toFixed(3)),
            ))}
            type="button"
          >
            <MinusIcon />
          </button>
          <output aria-live="polite" className={quantityOutputClass} key={quantity}>{quantity}</output>
          <button
            aria-label={t("increaseQuantity")}
            className={quantityButtonClass}
            disabled={quantity >= product.stockQuantity}
            onClick={() => setQuantity((value) => Math.min(
