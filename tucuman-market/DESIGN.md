...
- На viewport от 1440px дополнительная круглая кнопка корзины может быть
  зафиксирована справа за пределами контейнера 1280px. Она открывает тот же
  cart drawer, показывает счётчик и не заменяет основную кнопку в header.
